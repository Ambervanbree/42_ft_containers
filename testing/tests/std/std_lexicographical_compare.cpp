...
/*   Updated: 2022/07/05 12:16:08 by avan-bre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TEST(std_lexicographical_compare, string){
	...
	EXPECT_FALSE(std::lexicographical_compare(string1.begin(), string1.end(),
		string1.begin(), string1.end()));
	EXPECT_FALSE(std::lexicographical_compare(string1.begin(), string1.end(),
		string2.begin(), string2.end()));
	EXPECT_TRUE(std::lexicographical_compare(string1.begin(), string1.end(),
...
TEST(std_lexicographical_compare, vector){	
	...
	EXPECT_FALSE(std::lexicographical_compare(vec1.begin(), vec1.end(),
		vec1.begin(), vec1.end()));
	EXPECT_FALSE(std::lexicographical_compare(vec1.begin(), vec1.end(),
		vec2.begin(), vec2.end()));
	EXPECT_TRUE(std::lexicographical_compare(vec1.begin(), vec1.end(),
		vec3.begin(), vec3.end()));
	EXPECT_FALSE(std::lexicographical_compare(vec1.begin(), vec1.end(),
		vec4.begin(), vec4.end()));
}
